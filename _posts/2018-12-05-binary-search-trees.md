...
- We create a new node with value `19` and add is as right child of `16`
...
Now that we know the algorithms, we can write the code:
...
class Bst {
  ...
  // Search for a value in this tree. If not found, returns the closest node
  // (the node where the value would be inserted)
  _findPosition(val) {
    ...
    while (true) {
      ...
      if (val > current.val && current.right) {
        // The value we are searching for is greater. Continue on the right
        // subtree
        current = current.right;
      } else if (val < current.val && current.left) {
        // The value we are searching for is lower. Continue on the left subtree
  ...
  // Deletes the given node. The node must have at must one child
  _deleteSimple(node) {
  ...
  insert(val) {
    ...
    // Search for the value. If it is found we don't need to do anything
  ...
  delete(val) {
    // Search for the value. If it is not found we don't need to do anything
    ...
    // If the node doesn't have children, just delete it
